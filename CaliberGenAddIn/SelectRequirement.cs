...
namespace EAAddIn
{
    public class SelectRequirements
    {
        ...
        public SelectorSerialTag FetchSelector(DataTable dt)
        {
            var selector = new SelectorSerialTag();

            foreach (TreeNode node in mainform.treeViewCaliber.Nodes)
            {
                AddTreeNodeToSelector(node, selector, dt);
            }
            return selector;
        }


        public void AddTreeNodeToSelector(TreeNode node, SelectorSerialTag selector, DataTable dt)
        {
            ...
            {
                AddTreeNodeToSelector(child, selector, dt);
            }
        }
                                    ...
                                    Repository m_Repository)
        {
            ...
            if (req != null)
            {
                ...
                description = Convert(exReq.Requirement.Description.Text);

                dr[COL_CALIBERFULLDESCRIPTION] = description;
                ...
                {
                    MessageBox.Show(dr[COL_CALIBERREQUIREMENT] + " has a duplicate Caliber ID (" + dr[COL_CALIBERID] + ") and WILL NOT BE IMPORTED." + Environment.NewLine + Environment.NewLine +
                        "This occurs when a branched requirement has been copied and reused." + Environment.NewLine + Environment.NewLine +
                        "Please reset the Branch Merge attributes of the copied requirement to 'Initial Value' and try again." + Environment.NewLine + Environment.NewLine +
                        "If you are unable to correct this problem yourself, please contact the BARM Team for assistance. " + Environment.NewLine + Environment.NewLine +
        ...
        private string ProcessLists(string description)
        {
            return ProcessLists(description, 0, false);
        }

        private string ProcessLists(string description, int depth, bool ordered)
        {
            int myDepth = depth + 1;
        ...
        private symbol GetNextSymbol(string description)
        {
        ...
        public bool GetDetails(string caliberId)
        {
            ...
            sqlCommand1 = EaCaliberGenEngine.MappingTableConnection.CreateCommand();
