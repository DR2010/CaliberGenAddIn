...
namespace EAAddIn
{
    public class EaCaliberGenEngine
    {
        ...
        WordReportEngine reportEngine = new WordReportEngine();
        ...
        public const string constCaliberProcessObject = "12.";
        ...
        public const string TYPE_ProcessObject = "Process Object";
        public const string TYPE_OR = "PR-";
        ...
        public static SqlConnection EadbConnection
         {
            get
            {
                if (_eadbConnection != null
                    && !string.IsNullOrEmpty(_eadbConnection.ConnectionString) 
                    && _eadbConnection.State == ConnectionState.Closed)
                {
                    _eadbConnection.Open();
                }

                return _eadbConnection;
            }
        }
        private static SqlConnection _eadbConnection;
        ...
        //
        public EaCaliberGenEngine(SecurityInfo sec, string loadType )
        {
        ...
        public static void updateCABConnections(List<callCompare> cabList)
        {
            ...
            foreach (callCompare cab in cabList)
            {
                if (cab.decision == "NEW ELEMENT CALLS")
                {
                    ...
                    gm = mtCABType.getStereotypeForGen(SqlHelpers.MappingDbConnection,
        ...
        public bool AddCabToList(Package parentPackage, XmlNode parent, TreeNode node)
        {
            ...
            //
            var gm = new mtCABType.genModule();
            gm = mtCABType.getStereotypeForGen(SqlHelpers.MappingDbConnection, module, name);
        ...
        private void EaAddTableConnector(XmlNode parent, XmlNode table)
        {
            ...
            for (short i = 0; i < EAfrom.Connectors.Count; i++)
            {
                ...
                {
                    found = true;
                    if (String.IsNullOrEmpty(con.Name) || con.Name == "unspecified")
                    {
                        con.Name = linkName;
                        con.Update();
                    }
        ...
        private void EaAddProgramConnector(XmlNode from, XmlNode to)
        {
            ...
            //
            var gm = new mtCABType.genModule();
            gm = mtCABType.getStereotypeForGen(SqlHelpers.MappingDbConnection, fromCab, fromCabName);
            ...
            gmTO = mtCABType.getStereotypeForGen(SqlHelpers.MappingDbConnection,
                                                 toCab, toCabName);
        ...
        // ----------------------------------------------
        public void MappingCaliberloadEaGuid( ToolStripProgressBar progress)
        {
            progress.Value = 0;
            progress.Step = 1;
            progress.Maximum = elementSourceDataTable.Rows.Count;
        ...
        public string CaliberEaSync(Action<string> status, ToolStripProgressBar progress)
        {
            updateStatus = status;
            ...
            bool createLinkedDocument = false;
            string linkedDocumentText = string.Empty;
            Element BDO;
            Element screen = null;
            string BDO_EAGUID = "";
            int previousLevel = 0;
                ...
                if (caliberSourceID > 0)
                {
                    ...
                    if (realCaliberIDMap.CaliberID > 0)
                    {
                        ...
                        if (eaEl != null)
                        {
                            ...
                            WriteResults("Requirement re-mapped after partial merge. " +
                                                         " From: " + realCaliberID +
                                                         " To: " + caliberID, caliberID);
                        }
                        else
                        {
                            WriteResults(
                ...
                if (derivedType == Derivedtype.Package.ToString())
                {
                    ...
                    if (eaInfo.EA_GUID == null)
                    {
                        ...
                        eaInfo.CaliberFullDescription = //dr["CaliberFullDescription"].ToString();
                            AddInRepository.Instance.Repository.GetFieldFromFormat("HTML", dr["CaliberFullDescription"].ToString());
                    ...
                    )
                {
                    ...
                    #region ExistingEAElement
                    {
                        ...
                        else
                        {
                            ...
                            if (
                                (EAElement.Notes == AddInRepository.Instance.Repository.GetFieldFromFormat("HTML", dr["CaliberFullDescription"].ToString()) ||
                                 ( EAElement.Notes == HtmlHelper.StripHtmlAdvanced(dr["CaliberFullDescription"].ToString()) && !string.IsNullOrEmpty(EAElement.GetLinkedDocument()))) &&
                            ...
                            else
                            {
                                ...
                                SetElementNotes(EAElement, dr["CaliberFullDescription"].ToString(), ref createLinkedDocument, ref linkedDocumentText);
                                EAElement.Status = dr["CaliberStatus"].ToString();
                                EAElement.Name = dr["CaliberRequirement"].ToString();
                            }
                                ...
                                || derivedType == Derivedtype.Businessruleparent.ToString())
                            {
                                if (EAElementParent != null)
                                {
                                    if (EAElementParent.Type == "Package")
                                        ...
                                        {
                                            ret =
                                                "Error setting parent info Element Name: " +
                                                EAElement.Name +
                                                "Parent Package ID: " +
                                                EAElementParent.PackageID;
                                            MessageBox.Show(ret);

                                            WriteResults( ret, caliberID);
                            ...
                            if (! UpdateElement(EAElement, ref createLinkedDocument, ref linkedDocumentText))
                            {
                    ...
                    #region Non-Existing EA Element
                    {
                            ...
                            || derivedType == Derivedtype.Businessruleparent.ToString())
                        {
                            if (EAElementParent != null)
                            {
                                if (EAElementParent.Type == "Package")
                                    ...
                                    {
                                        ret = "Error setting parent info Element Name: " +
                                              newEAElement.Name +
                                              "Parent Package ID: " +
                                              EAElementParent.PackageID;
                                        MessageBox.Show(ret);
                        ...
                        SetElementNotes(newEAElement, dr["CaliberFullDescription"].ToString(), ref createLinkedDocument, ref linkedDocumentText);

                        newEAElement.Status = dr["CaliberStatus"].ToString();
                        UpdateElement(newEAElement, ref createLinkedDocument, ref linkedDocumentText);
                    ...
                    )
                {
                    ...
                    if (eaInfo.EA_GUID != null)
                    {
                        ...
                        else
                        {
                            ...
                            //
                            UpdateElement(EAElement, ref createLinkedDocument, ref linkedDocumentText);
                            EAElement.Refresh();
                            ownerPackage.Elements.Refresh();
...

                    {
...

                        {
                            ...
                            SetElementNotes(screen, dr["CaliberFullDescription"].ToString(), ref createLinkedDocument, ref linkedDocumentText);
                            screen.Status = dr["CaliberStatus"].ToString();
                            UpdateElement(screen, ref createLinkedDocument, ref linkedDocumentText);
                            screen.Refresh();
                            ownerPackage.Elements.Refresh();
        ...
        private void WriteResults(string message, int caliberID)
        {
            AddInRepository.Instance.WriteCaliberResults(message, caliberID);
        ...
        private void SetElementNotes(Element element, string value, ref  bool createLinkedDocument, ref string linkedDocumentText)
        {
            if (value.ToUpper().Contains("<BLOCKQUOTE"))
            {
                linkedDocumentText = value;
                value = value.Replace("<blockquote>", string.Empty);
                value = value.Replace("</blockquote>", string.Empty);
                createLinkedDocument = true;
            }
            
            if (value.ToUpper().Contains("<TABLE"))
            {
                element.Notes = HtmlHelper.StripHtmlAdvanced(value);
                createLinkedDocument = true;
                linkedDocumentText = value;
            }
            else
            {
                element.Notes = AddInRepository.Instance.Repository.GetFieldFromFormat("HTML", value);
            }
        }

        private bool UpdateElement(Element element, ref  bool createLinkedDocument, ref string linkedDocumentText)
        {
            if (createLinkedDocument)
            {
                HtmlHelper.CopyHtmlToClipBoard(linkedDocumentText);
                var file = reportEngine.CreateReport();
                element.LoadLinkedDocument(file);
            }
            var ok = element.Update();

            createLinkedDocument = false;

            linkedDocumentText = string.Empty;
                                 ...
                                 SelectRequirements.ElementProperties uiProperties, int caliberID)
        {
            ...
            WriteResults(eaElement.Name + ": Updating element properties", caliberID);
        ...
        // ------------------------------------------------------
        public void DisplayEaTreePreview(TreeView treeViewEA, ToolStripProgressBar progress)
        {
            // Image list
            //
        ...
        //                              Get element image
        // -------------------------------------------------------------------
        public static int GetImageId(string type, bool existsInEA)
        {
            int image = 7;
        ...
        // -------------------------------------------------------------------
        public static Package CreateEaPackage(Package parentPackage, string packageName)
        {
